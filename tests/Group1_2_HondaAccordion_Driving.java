...
public class Group1_2_HondaAccordion_Driving extends BCATestScenario {
    ...
    public int runTest() {
        ...
        honda.drive(166);
        assertEquals(honda.getRemainingRange(), 14.5 * 33.2 - 166, .1, "Invalid remaining range");
        assertEquals(honda.getFuelLevel(), 9.5, .1, "Invalid fuel level");
        assertThrows(IllegalArgumentException.class, () -> honda.drive(315.41), "Honda should not be able to drive 315.41");
        honda.drive(315.4);
        assertEquals(honda.getMileage(), 200 + 14.5 * 33.2, .1, "Mileage does not increment correctly");
        return 0;
    }
