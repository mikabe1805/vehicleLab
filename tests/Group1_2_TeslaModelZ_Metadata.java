...
public class Group1_2_TeslaModelZ_Metadata extends BCATestScenario {
    ...
    public int runTest() {
        ...
        assertEquals(tz.getMaxRange(), 340.0, .1, "Incorrect max range");
        assertEquals(tz.getRemainingRange(), 340.0, .1, "Incorrect remaining range");

        assertEquals(tz.getMake(), "Tesla", "Incorrect manufacturer");
        assertEquals(tz.getMileage(), 0, .1, "Incorrect starting mileage");
